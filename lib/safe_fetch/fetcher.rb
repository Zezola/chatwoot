class SafeFetch::Fetcher
  ...
  def request_stream(&)
    return direct_request_stream(&) if options.private_host_allowed?

    SsrfFilter.public_send(options.method, options.url, **options.request_options, &)
  end

  def direct_request_stream
    request = Net::HTTP.const_get(options.method.to_s.capitalize).new(options.uri)
    options.headers&.each { |key, value| request[key] = value }
    request.body = options.body if options.body.present?
    options.apply_basic_authentication(request)

    Net::HTTP.start(
      options.uri.hostname,
      options.uri.port,
      use_ssl: options.uri.is_a?(URI::HTTPS),
