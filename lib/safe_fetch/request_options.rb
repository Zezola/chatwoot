class SafeFetch::RequestOptions
  ...
  attr_reader :allowed_content_type_prefixes, :allowed_content_types, :body, :headers,
              :allowed_private_hosts, :http_basic_authentication, :method, :open_timeout, :read_timeout, :uri, :url
  ...
  def initialize(url:, **options)
    ...
    @allowed_private_hosts = normalize_allowed_private_hosts(config[:allowed_private_hosts])
  ...
  def filename
    @filename ||= normalized_filename || "download-#{Time.current.to_i}-#{SecureRandom.hex(4)}"
  end
  ...
  def private_host_allowed?
    allowed_private_hosts.include?(uri.hostname.to_s.downcase)
  end

  def apply_basic_authentication(request)
    credentials = basic_authentication_for(request.uri)
  ...
  def normalize_allowed_private_hosts(value)
  ...
  def normalized_filename
    ...
    return if filename.blank? || ['.', '/'].include?(filename)
  ...
  def basic_authentication_for(request_uri)
    explicit_basic_authentication_for(request_uri) || uri_basic_authentication(request_uri) || original_uri_basic_authentication(request_uri)
  end

  def explicit_basic_authentication_for(request_uri)
    return unless same_origin?(request_uri, uri)

    http_basic_authentication.presence
  end
