...
RSpec.describe 'Devise::Mailer' do
  describe 'notify' do
    ...
    it 'shows the default confirmation state' do
      ...
      expect(mail_body).to include('Boas-vindas ao Chatwoot. Precisamos confirmar seu endereço de e-mail antes de liberar o acesso à conta.')
      expect(mail_body).to include('Confirmar minha conta')
      expect(mail_body).not_to include('Convite para o workspace')
    end
    ...
    context 'when there is an inviter' do
      ...
      it 'refers to the inviter and their account' do
        expect(mail_body).to include("Você foi convidado para #{account.name}")
        expect(mail_body).to include("#{inviter_val.name} convidou você para entrar no workspace #{account.name} no Chatwoot.")
