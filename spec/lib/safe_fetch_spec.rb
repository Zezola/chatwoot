...
RSpec.describe SafeFetch do
  ...
  describe '.fetch' do
    ...
    context 'with SSRF protection (integration with ssrf_filter)' do
      ...
      it 'allows an explicitly trusted private hostname' do
        trusted_url = 'http://app.local/webhook'
        allow(Resolv).to receive(:getaddresses).with('app.local').and_return(['172.19.0.5'])
        stub_request(:post, trusted_url).to_return(status: 200, body: '', headers: {})

        expect do
          described_class.fetch(
            trusted_url,
            method: :post,
            allowed_private_hosts: 'app.local',
            validate_content_type: false
          ) { nil }
        end.not_to raise_error
      end

      it 'keeps blocking private hosts that are not explicitly trusted' do
        allow(Resolv).to receive(:getaddresses).with('postgres').and_return(['172.19.0.10'])
        expect do
          described_class.fetch('http://postgres:5432/secret', allowed_private_hosts: 'app.local') { nil }
        end
