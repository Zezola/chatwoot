...
describe Webhooks::Trigger do
  ...
  describe '#execute' do
    ...
    context 'when webhook type is agent bot' do
      ...
      [502, 503, 504].each do |status|
        it "raises #{status} errors for retry and does not reopen conversation immediately" do
          payload = { event: 'message_created', id: pending_message.id }

          expect(SafeFetch).to receive(:fetch).and_raise(SafeFetch::HttpError.new("#{status} transient error"))
      ...
      it 'raises network failures for retry and does not reopen conversation immediately' do
        payload = { event: 'message_created', id: pending_message.id }
      ...
      it 'keeps pending conversation unchanged after a final non-retryable failure' do
        payload = { event: 'message_created', id: pending_message.id }
      ...
      it 'keeps conversation pending even when keep_pending_on_bot_failure setting is disabled' do
  ...
  context 'when webhook timeout is set via environment' do
    it 'uses the environment value before global config' do
      with_modified_env WEBHOOK_TIMEOUT: '50' do
        expect(SafeFetch).to receive(:fetch).with(
          url,
          method: :post,
          body: payload.to_json,
