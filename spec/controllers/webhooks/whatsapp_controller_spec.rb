...
RSpec.describe 'Webhooks::WhatsappController', type: :request do
  ...
  describe 'POST /webhooks/whatsapp/{:phone_number}' do
    ...
    it 'finds the channel by phone number id before validating the Meta signature' do
      channel_secret = 'channel-whatsapp-secret'
      channel.update!(
        phone_number: '+5511995557780',
        provider_config: channel.provider_config.merge('app_secret' => channel_secret)
      )
      ...
      post_whatsapp_webhook(
        '/webhooks/whatsapp/+551195557780',
        channel_body,
        signature: signature_for(channel_body, channel_secret),
        env: {}
      )
    ...
    it 'normalizes known Brazilian phone fields before enqueuing the webhook job' do
      allow(Webhooks::WhatsappEventsJob).to receive(:perform_later)

      channel_body = {
        phone_number: '+551195557780',
        object: 'whatsapp_business_account',
        entry: [{
          changes: [{
            value: {
              ...
              contacts: [{ wa_id: '551195557780' }],
              messages: [{ from: '551195557780', type: 'text', text: { body: 'hello' } }],
              message_echoes: [{ from: '551195557780', to: '551195557781', text: { body: 'echo' } }]
            }
          }]
        }]
      }.to_json

      post_whatsapp_webhook('/webhooks/whatsapp/+551195557780', channel_body)

      expect(response).to have_http_status(:success)
      expect(Webhooks::WhatsappEventsJob).to have_received(:perform_later) do |payload|
        change_value = payload[:entry].first[:changes].first[:value]
        expect(payload[:phone_number]).to eq('+5511995557780')
        expect(change_value[:metadata][:display_phone_number]).to eq('+5511995557780')
        expect(change_value[:contacts].first[:wa_id]).to eq('5511995557780')
        expect(change_value[:messages].first[:from]).to eq('5511995557780')
        expect(change_value[:message_echoes].first[:from]).to eq('5511995557780')
        expect(change_value[:message_echoes].first[:to]).to eq('5511995557781')
      end
    end
    ...
    context 'when phone number is in inactive list' do
      ...
      it 'matches inactive phone numbers without plus after normalization' do
        allow(GlobalConfig).to receive(:get_value).with('INACTIVE_WHATSAPP_NUMBERS').and_return('5511995557780')

        post_whatsapp_webhook('/webhooks/whatsapp/551195557780', body)
