class Webhooks::WhatsappEventsJob < MutexApplicationJob
  ...
  def contact_sender_id(params)
    ...
    normalized_phone_number(message[:to] || message[:from])
  end
  ...
  def find_channel_from_whatsapp_business_payload(params)
    ...
    return find_channel_by_url_param(params) unless params[:object] == 'whatsapp_business_account'

    get_channel_from_wb_payload(params) || find_channel_by_url_param_when_phone_number_id_is_blank(params)
  end
  ...
  def find_channel_by_url_param_when_phone_number_id_is_blank(params)
    metadata = whatsapp_business_metadata(params)
    return if metadata.blank? || metadata[:phone_number_id].present?

    find_channel_by_url_param(params)
  end

  def whatsapp_business_metadata(params)
    params.dig(:entry, 0, :changes, 0, :value, :metadata)
  end
